...
import { FoodItem, FoodLog, WorkoutLogged, Client, ComplianceStats } from './types';
...
const App: React.FC = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        console.log("MuskyFit [System]: Initializing Boot Sequence...");
        ...
        if (profile) {
          console.log("MuskyFit [Auth]: Profile Identified -", profile.email);
          setCurrentUser(profile);
          setNeedsOnboarding(false);
          const stats = await sheetsService.getCompliance(profile.id);
          setComplianceStats(stats);
        } else {
          console.log("MuskyFit [Auth]: No Profile Found. Directing to Onboarding.");
          setNeedsOnboarding(true);
        }
      } catch (err) {
        console.error("MuskyFit [Error]: Initialization Failure -", err);
        setNeedsOnboarding(true);
      } finally {
    ...
    return (
      <div className="fixed inset-0 bg-musky-black z-[200] flex flex-col items-center justify-center p-6 overflow-hidden">
        ...
        <div className="text-center space-y-6">
          <h1 className="text-white text-4xl font-black tracking-[0.4em] uppercase">MUSKYFIT</h1>
  ...
  return (
    <div className="min-h-screen bg-musky-black flex flex-col">
      ...
      <nav className="fixed bottom-0 left-0 right-0 bg-musky-black/80 musky-blur border-t-2 border-white/10 z-[100] h-24 pb-safe">
        <div className="max-w-md mx-auto grid grid-cols-5 h-full">
          ...
          <TabButton active={activeTab === 'nutrition'} onClick={() => setActiveTab('nutrition')} label="FUEL" />
...
const TabButton = ({ active, onClick, label }: { active: boolean, onClick: () => void, label: string }) => (
  <button 
    onClick={onClick}
    className={`flex flex-col items-center justify-center gap-2 transition-all duration-300 ${active ? 'text-musky-orange' : 'text-white/20'}`}
  >
    <div className={`h-[2px] w-4 transition-all duration-500 ${active ? 'bg-musky-orange w-8' : 'bg-transparent'}`}></div>
